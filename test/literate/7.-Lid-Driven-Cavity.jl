...
#-
halflength=0.5 # make rectangle with side length of 1
domain_lim=halflength+1.01*Δx; # 1.01 is just an abitrary factor chosen to be greater than 1
#-
xlim = (-domain_lim,domain_lim)
ylim = (-domain_lim,domain_lim)
...
plot(
plot(vorticity(integrator),sys,title="Vorticity (Computed)",clim=(-5,5),levels=10,color=:turbo,linewidth=1.5,ylim=ylim,fillrange=nothing),
plot(streamfunction(integrator),sys,title="Streamlines (Computed)",fillrange=nothing, color=:black),
   size=(700,300))
